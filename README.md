...
ログインユーザーごとに「スケジュール」「TODOリスト」「日記」を統合管理できるWebアプリケーションです。

---

## 💡 概要・目的

日常のタスクやスケジュール、日記を1つの画面でストレスなく管理することを目的に作成しました。
...
スケジュール・TODO・日記を複数のアプリに分けず、1つの画面でシンプルに一括管理したい個人ユーザー
...
* **UI/UX・操作性の向上**
  * カレンダーの「日曜日始まり」への切り替え対応
  * TODOリストのドラッグ＆ドロップ等による並び順変更機能
  * 日記投稿時の任意日付（過去・未来）指定機能
* **アカウント管理機能の拡張**
  * 退会（アカウント削除）機能および登録情報の変更（パスワード変更等）機能
* **通知・リマインド機能**
  * TODOの期限やスケジュールのブラウザ通知
* **法人・チーム利用への拡張（チームビルディング対応）**
  * 個人向け「日記」から組織向け「日報」への機能拡張
  * チーム内での日報・スケジュール共有およびコメント機能
* **クラウド環境へのデプロイ**
  * Render / Fly.io 等を活用したWeb上でのオンライン公開
